...
impl Clock {
    pub fn new(hours: i32, minutes: i32) -> Self {
        // additional hours to add/subtract from minutes
        let add_hours = Self::additional_hours(minutes);
        let actual_hour = match (hours + add_hours) % 24 {
            h if h.is_negative() => h + 24,
            h => h,
        };
        let actual_minute = match minutes % 60 {
            m if m.is_negative() => m + 60,
            m => m,
        };
    ...
    pub fn add_minutes(&self, minutes: i32) -> Self {
        ...
        let add_hours = Self::additional_hours(total_minutes);
        let actual_hour = match (self.hours + add_hours) % 24 {
            h if h.is_negative() => h + 24,
            h => h,
        };
        let actual_minute = match (total_minutes) % 60 {
            m if m.is_negative() => m + 60,
            m => m,
        };
    ...
    // computes additional hours to add to the clock based on the minutes value
    fn additional_hours(minutes: i32) -> i32 {
        match minutes {
            _ if minutes.is_positive() => minutes / 60,
            _ => (minutes as f32 / 60.0).floor() as i32,
        }
    }
}
