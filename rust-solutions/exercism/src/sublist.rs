...
pub fn sublist<T: PartialEq>(_first_list: &[T], _second_list: &[T]) -> Comparison {
    match ( contains_sublist(_first_list, _second_list), contains_sublist(_second_list, _first_list) ) {
...
fn contains_sublist<T: PartialEq>(als: &[T], bls: &[T]) -> bool {
    ...
    if als.is_empty() { return true; }

    // find all indices of als[0] in bls
    let indices: Vec<usize> = bls.iter().enumerate()
